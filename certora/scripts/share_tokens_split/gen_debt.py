...
from share_tokens_split_rule_names import rule_names
...
def generate_config(rule_name):
    config = {
        ...
        "verify": "Debt1:certora/specs/share_tokens_split.spec"
    }
    
    os.makedirs("debt", exist_ok=True)
    filename = os.path.join("debt", f"Debt1_{rule_name}_verified.conf")
